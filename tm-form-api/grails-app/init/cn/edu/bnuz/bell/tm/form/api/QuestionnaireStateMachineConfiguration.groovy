...
@Configuration
@EnableStateMachine(name='questionnaireStateMachine')
@Import(StandardActionConfiguration)
class QuestionnaireStateMachineConfiguration extends EnumStateMachineConfigurerAdapter<State, Event> {
    @Autowired
    ...
    void configure(StateMachineStateConfigurer<State, Event> states) throws Exception {
        states
            .withStates()
                ...
                .state(State.SUBMITTED, [actions.logEntryAction(), questionnaireSubmittedEntryAction()],  [actions.workitemProcessedAction()])
    ...
    @Bean
    Action<State, Event> questionnaireSubmittedEntryAction() {
        new QuestionnaireSubmittedEntryAction()
    }

    @Bean
    Action<State, Event> questionnairePublishedEntryAction() {
        new AutoEntryAction()
    }
}
